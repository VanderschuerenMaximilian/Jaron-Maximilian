"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BezoekerService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const bezoeker_entity_1 = require("./entities/bezoeker.entity");
const typeorm_2 = require("typeorm");
let BezoekerService = exports.BezoekerService = class BezoekerService {
    constructor(bezoekerRepository) {
        this.bezoekerRepository = bezoekerRepository;
    }
    create(createBezoekerInput) {
        const b = new bezoeker_entity_1.Bezoeker();
        b.name = createBezoekerInput.name;
        b.fullname = createBezoekerInput.fullname;
        b.email = createBezoekerInput.email;
        return this.bezoekerRepository.save(b);
    }
    findAll() {
        return this.bezoekerRepository.find();
    }
    findOne(id) {
        return `This action returns a #${id} bezoeker`;
    }
    update(id, updateBezoekerInput) {
        return `This action updates a #${id} bezoeker`;
    }
    remove(id) {
        return `This action removes a #${id} bezoeker`;
    }
    save(bezoeker) {
        return this.bezoekerRepository.save(bezoeker);
    }
    truncate() {
        return this.bezoekerRepository.clear();
    }
};
exports.BezoekerService = BezoekerService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(bezoeker_entity_1.Bezoeker)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], BezoekerService);
//# sourceMappingURL=bezoeker.service.js.map