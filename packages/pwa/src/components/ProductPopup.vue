            ...
            <div class="flex flex-col">
                <div class="flex absolute h-screen w-screen justify-center">
                    <div class="relative z-10 min-w-screen sm:min-w-120 flex w-35vw h-70vh mt-30 bg-slate-100 border-t-14 border-primary-green rounded-lg">
                        <button @click="closePopup" class="absolute right-4 top-4 button-focus rounded-full">
                            <X class="w-6 h-6 text-primary-green hover:opacity-80" />
                        </button>
                        <div class="flex flex-col mx-auto w-80%">
                            ...
                            <div class="flex flex-col h-500 max-h-46vh overflow-auto px-4 mt-1">
                                ...
                                <select v-if="selectedProduct.category !== 'Burgers'" v-model="selectedSize" class="p-2 mt-2 border-3 border-primary-green hover:opacity-80 rounded-md cursor-pointer button-focus">
                                ...
                                <select v-if="selectedProduct.category === 'Burgers'" v-model="selectedSauce" class="p-2 mt-2 border-3 border-primary-green hover:opacity-80 rounded-md cursor-pointer button-focus">
                                ...
                                <div  v-for="extra in selectedProduct.extra" class="flex flex-col">
                                    <label v-if="extra.stock >= extra.stockReduction && listExtras.filter((item: any) => item.ingredient === extra.name).map((item: any) => item.stock)[0] !=0" :key="extra.name" class="flex items-center mt-1 cursor-pointer select-none">
                                        <input v-if="extra.stock >= extra.stockReduction" type="checkbox" v-model="selectedToppings" :value="extra" class="mr-2 cursor-pointer button-focus rounded-lg">
                                        {{ extra.name }}
                                    </label>
                                </div>
                                ...
                                <div class="flex flex-col">
                                    <label v-for="removable in selectedProduct.removable" :key="removable" class="flex items-center mt-1 cursor-pointer select-none">
                                        <input type="checkbox" v-model="selectedRemovables" :value="removable" class="mr-2 cursor-pointer button-focus">
                                        {{ removable }}
                                    </label>
                                </div>
                            </div>
                            <button @click="handleData" class="bottom-4 bg-primary-green my-5 text-slate-200 mx-20 font-bold py-3 rounded-lg hover:opacity-80 button-focus">Add to card</button>
                        </div>
                    </div>
                </div>
            </div>
